# -*- coding: utf-8 -*-
"""
Created on Wed Nov 17 04:17:33 2021
...
def tidyGame(inputGame):
    ...
    #df2 = df[df["gameType"]=="P"]
    df2 = df[df["gameType"]!="R"]
    #df2 = df2[df2["season"]!=2019]
    ...
    a=inputGame["liveData"]["plays"]["penaltyPlays"]
    c=inputGame["liveData"]["plays"]["allPlays"]
    ...
    for r,x,y,x2,y2,e in zip(df2["rinkSide"],df2["xCoord"],df2["yCoord"],df2["lastEventXCoord"],df2["lastEventYCoord"],df2["lastEventType"]):
        if str(e)=="Shot":
            rebound.append(True)
        else:
...
for x in data1:
    t=tidyGame(x)
    if type(t)!=type(np.nan):
        bigList.append(t)


dfTotal=dfConcat(bigList)
dft1=dfTotal[dfTotal["season"]!=2019]
dft2=dfTotal[dfTotal["season"]==2019]
#dft1.to_csv("trainingSet.csv",index=False)
#dft2.to_csv("testSet.csv",index=False)
dft1.to_csv("trainingSetPlayOff.csv",index=False)
dft2.to_csv("testSetPlayOff.csv",index=False)